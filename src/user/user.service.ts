...
export class UserService {
    constructor(
      ...
      async createUserProfile(createUserProfileDto: CreateUserProfileDto): Promise<User> {
        const logContext = {
        ...
        try {
            ...
            const existingUser = await this.findUserBySupabaseId(createUserProfileDto.supabaseUserId);
            if (existingUser) {
                this.loggerService.warn('Attempted to create user with existing Supabase ID', logContext);
                throw new BadRequestException('User with this Supabase ID already exists');
            }

            // Check if email already exists
            const existingEmail = await this.findUserByEmail(createUserProfileDto.email);
            if (existingEmail) {
                this.loggerService.warn('Attempted to create user with existing email', logContext);
            ...
            this.loggerService.error('Failed to create user profile', error, logContext);
            throw new Error(`Failed to create user profile: ${error.message}`);
        }
      }

      async updateUserProfile(supabaseUserId: string, updateUserProfileDto: UpdateUserProfileDto): Promise<User> {
        try {
            ...
            const updateData = {
                ...
                dateOfBirth: updateUserProfileDto.dateOfBirth ? new Date(updateUserProfileDto.dateOfBirth) : user.dateOfBirth,
            };
      ...
      async linkSupabaseId(userId: number, supabaseUserId: string): Promise<User> {
        ...
        try {
            ...
            this.loggerService.info('Supabase ID linked to existing user', logContext);
      ...
      async createFromSupabasePayload(payload: any): Promise<User> {
        ...
        try {
            ...
            this.loggerService.error('Failed to create user from Supabase payload', error, logContext);
            throw new Error(`Failed to create user from Supabase payload: ${error.message}`);
        }
      }
}
