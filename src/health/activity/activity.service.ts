...
import { ActivityMetric, ActivityType } from './activity-metric.entity.js';
...
export class ActivityService {
  ...
  async create(
    userId: number,
    createActivityMetricDto: CreateActivityMetricDto,
    ...
    try {
      ...
      const savedActivityMetric =
        await this.activityMetricRepository.save(activityMetric);
      ...
      this.loggerService.error(
        'Failed to create activity metric',
        error,
        logContext,
      );
  ...
  async update(
    id: number,
    userId: number,
    updateActivityMetricDto: UpdateActivityMetricDto,
  ): Promise<ActivityMetric> {
    const logContext = {
      userId,
    ...
    try {
      ...
      const updateData = {
        ...
        date: updateActivityMetricDto.date
          ? new Date(updateActivityMetricDto.date)
          : activityMetric.date,
      };
    ...
    try {
      ...
      if (activityMetrics.length === 0) {
        this.loggerService.warn(
          'No activity data found for stats calculation',
          logContext,
        );
      ...
      const totalDuration = activityMetrics.reduce(
        (sum, activity) => sum + activity.duration,
        0,
      );
      const totalCaloriesBurned = activityMetrics.reduce(
        (sum, activity) => sum + (activity.caloriesBurned || 0),
        0,
      );
      const totalDistance = activityMetrics.reduce(
        (sum, activity) => sum + (activity.distance || 0),
        0,
      );
      const totalSteps = activityMetrics.reduce(
      ...
      const averageIntensity =
        Math.round(
          (intensityScores.reduce((a, b) => a + b, 0) /
            intensityScores.length) *
            100,
        ) / 100;

      const heartRates = activityMetrics
      ...
      const activityTypeBreakdown =
        this.calculateActivityTypeBreakdown(activityMetrics);

      // Weekly progress
      const weeklyProgress = this.calculateWeeklyProgress(activityMetrics);
      ...
      this.loggerService.error(
        'Failed to calculate activity stats',
        error,
        logContext,
      );
  ...
  private calculateActivityTypeBreakdown(activities: ActivityMetric[]) {
    ...
    activities.forEach((activity) => {
      ...
      breakdown[activity.activityType].totalCalories +=
  ...
  private calculateWeeklyProgress(activities: ActivityMetric[]) {
    const weeks = new Map<
      string,
      { totalDuration: number; totalActivities: number }
    >();
